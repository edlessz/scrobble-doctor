...
const doSearch = async (username, api_key) => {
  ...
  const done = (tracks) => {
    ...
    $("#status").innerHTML =
      `${new Date().toString()}<br>Tracks (${tracks.length}):`;
  };
  ...
  lastfm({
    ...
    .then((data) => {
      ...
      const startPage = checked
        ? 1
        : Math.max(1, Math.floor(parseInt($("#form_start").value) / limit) + 1);
      const endPage = checked
        ? pages
        : Math.min(
            pages,
            Math.floor(parseInt($("#form_end").value) / limit) + 1,
          );
      ...
      for (let i = startPage; i <= endPage; i++) {
        const resFunc = (data) => {
          ...
          const filteredTracks = data.recenttracks.track.filter(
            (track) => !track.album["#text"],
          );
          tracks = tracks.concat(
            filteredTracks.map(
              (x) =>
                `https://last.fm/user/${username}/library${x.url.substring(19)}`,
            ),
          );
          loaded++;
          $("#status").innerHTML =
            `Loading... ${Math.round((loaded / (endPage - startPage)) * 100)}%`;
...
const catchError = (e) => {
  $("#status").innerHTML =
    "An error occurred. Please check your username and API key.";
  console.error(e);
  errored = true;
};
...
($("#form_check_all").oninput = () => {
  const checked = $("#form_check_all").checked;
  $$(".check_all_toggle").forEach(
    (elm) => (elm.style.display = checked ? "none" : "block"),
  );
})();
